...
# My guesses
# wilrich = {'name': 'wil', 'vp': 3000.0, 'vs': 1800.0, 'rho': 2400.0, 'phi': 0.05, 'dz': 0.0}
# blueskyBIT = {'name': 'blBIT', 'vp': 2850.0, 'vs': 1700.0, 'rho': 2090.0, 'phi': 0.28, 'Kg':36.0*1000.0*1000.0*1000.0, 'dz': 28.0}       # From logs. Kg in GPa
# debolt = {'name': 'deb', 'vp': 4350.0, 'vs': 2300.0, 'rho': 2500.0, 'phi': 0.05, 'dz': 0.0}
# H2O = {'name': 'H2O', 'vp': 1550.0, 'rho': 959.0}    # SI units kg/m/second http://www.nist.gov/srd/upload/NISTIR5078-Tab3.pdf
GAS = {'name': 'GAS', 'vp': 490, 'rho': 18.5}      # http://www.peacesoftware.de/einigewerte/calc_methan.php5
# BIT = {'name': 'BIT', 'vp': 1350.0, 'rho': 940.0}   # Mochinaga et al  at 100C
...
virgin = M.reservoir([[wilrich, 2], [blueskyBIT, 28.0], [debolt, 2.0]], DIGI)
BDBEF = M.reservoir([[wilrich, 2], [blueskyGAS, 12.0], [blueskyBIT, 16.0], [debolt, 2.0]], DIGI)   # Blowdown before seis movie
BDAFT = M.reservoir([[wilrich, 2], [blueskyGAS, 24.0], [blueskyBIT, 4.0], [debolt, 2.0]], DIGI)   # Blowdown after seis movie
HSDUPPER = M.reservoir([[wilrich, 2], [blueskyH2O, 18.0], [blueskyGAS, 8.0], [blueskyBIT, 2.0], [debolt, 2.0]], DIGI)  # Top flood
HSDLOWER = M.reservoir([[wilrich, 2], [blueskyH2O, 26.0], [blueskyBIT, 2.0], [debolt, 2.0]], DIGI)  # Base flood
reservoirs = [virgin, BDBEF, BDAFT, HSDUPPER, HSDLOWER]

print 'bluegas: ', blueskyGAS

# virgin = reservoir([[wilrich, 0], [blueskyBIT, 30.0], [debolt, 0.0]], DIGI)
# BDBEF = reservoir([[wilrich, 0], [blueskyBIT, 29.5], [debolt, 0.0]], DIGI)
# BDAFT = reservoir([[wilrich, 0], [blueskyBIT, 29.0], [debolt, 0.0]], DIGI)
# HSDUPPER = reservoir([[wilrich, 0], [blueskyBIT, 28.5], [debolt, 0.0]], DIGI)
# HSDLOWER = reservoir([[wilrich, 0], [blueskyBIT, 28.0], [debolt, 0.0]], DIGI)
...
rmstop_base = virgin['rmstop']; rmsbase_base = virgin['rmsbase']; tt_base = virgin['tt']
#rmstop_base = BDAFT['rmstop']; rmsbase_base = BDAFT['rmsbase']; tt_base = BDAFT['tt']
for r in reservoirs:
    ...
    timeshift.append(DIGI*(tt_base - r['tt']))
